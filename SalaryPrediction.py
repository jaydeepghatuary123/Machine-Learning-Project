...
def main():
    ...
    try:
        ...
        print("Model Creation is Progress.")        
        regressionObject=LinearRegression()
        regressionObject.fit(X_train,Y_train)
        print("Model is Created")
        print("Press ENTER Key to Predict test data iin trained model ")
        input()
        ...
        print("Press ENTER Key see above result in graphical format")
        input()

        #Graph for Prdiction Value
        graph(X_train,Y_train,regressionObject,X_test,Y_test,Y_pred)

        r2=r2_score(Y_test,Y_pred)
        print("Our model i %2.2f%% accurate " %(r2*100))
